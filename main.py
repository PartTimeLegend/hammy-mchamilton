...
class QuestionManager:
    ...
    def get_random_question(self, quiz_type):
        if quiz_type not in self.questions:
            return None, None
        question, answer = random.choice(list(self.questions[quiz_type.capitalize()].items()))
        return question, answer

class Quiz:
    def __init__(self, questions):
        self.question_manager = QuestionManager(questions)

    async def start(self, message, quiz_type):
        question, answer = self.question_manager.get_random_question(quiz_type)
        if not question:
        ...
        try:
            response = await message.client.wait_for('message', timeout=20.0, check=check)
...
class QuizBot(discord.Client):
    def __init__(self, intents, questions):
    ...
    async def on_message(self, message):
        ...
        if message.content.startswith('!quiz'):
            ...
            await self.quiz.start(message, quiz_type)
